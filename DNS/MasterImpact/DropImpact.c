...
#define poolHeight 16.0                // Pool height (in radii)
#define domainSize 32.0                // Computational box size (in radii)
...
// Free slip and impermeability conditions
...
// Default for bottom is symmetry - axi need not be specified, 2D maybe does
u.n[bottom] = dirichlet(0.); // Impermeability
u.t[bottom] = neumann(0.); // Slip
...
event init (t = 0.0) {
  ...
  refine (((sq(x - (filmHeight + 2.0)) + sq(y) < sq(1.0*1.05) && sq(x - (filmHeight + 2.0)) + sq(y) > sq(1.0*0.95)) || fabs(x - filmHeight) <= 0.005) && level < maxLevel);
  
  // Create active liquid phase as union between drop and film
  fraction (f1, sq(1.0) - sq(x - (filmHeight + 2.0)) - sq(y));
...
event adapt (i++) {
  ...
  adapt_wavelet ((scalar *){f1, f2, u}, (double[]){1e-6, 1e-6, 1e-3, 1e-3}, maxLevel, minLevel);

  // Unrefine far away from the impact site
  // unrefine((y > 5.0) && level > minLevel);
